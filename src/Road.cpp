...
Road::Road(int SegmentHeight, int SegmentWidth, int SegmentLength, float TrackLength, float HorizonY)
...
void Road::render(SDL_Renderer* renderer, Uint32 deltaTime, Camera& camera) {
    ...
    for (size_t i = 0; i < segments.size() - 1; i++) {
        ...
        float relativeZ = fmod(seg.worldZ - camera.z + trackLength, trackLength);
        float nextRelativeZ = fmod(nextSeg.worldZ - camera.z + trackLength, trackLength);
